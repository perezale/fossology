...
static bool scanner_file_proc(scanner_object_t *s, char *path)
{
    ...
    else
    {
        ...
        *wfp_buffer = 0;
        ...
        scanner_wfp_capture(src, length, path, NULL, wfp_buffer);
    }
...
void scanner_wfp_capture(char * src, int length, char * path, char **md5, char *wfp_buffer)
{
    ...
    log_trace("WFP CAPTURE - length %d", length);
    if (md5 == NULL)
    {
...
{
    log_debug("Call API and save the response in: %s", s->curl_temp_path);
    curl_request(API_REQ_POST,"scan/direct",wfp,s);
    return NULL;
}
